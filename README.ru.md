...
Проект реализовн на Python с использованием библиотек OpenCV, Pillow и MoviePy.

Список автоматизированных действий:

1. Выборка изображений и их предобработка, представляющая собой удаление простого фона с помощью оператора Собеля и морфологических операций.
2. Трансформация изображений: масштабирование, поворот, зеркальное отображение.
...
Также в исследовательских целях была реализована пользовательская функция, вычисляющая градиент на основе оператора Собеля с ядрами размерности 3x3.

Для демонстрации возможностей программы был создан мультфильм (сюжет и сценарий являются авторскими, используемые изображения отрисованы мной в Procreate).

https://github.com/user-attachments/assets/e9df2880-2f0e-4584-ada1-aa6796b05d79

## Пример готовой анимации

Созданный посредством данной программы мультфильм длится 2 минуты 8 секунд и демонстрирует:

- осмысленный завершенный сюжет, стилизованный под сказку;
- несколько локаций;
- набор персонажей и «реквизита»;
- 12 наполненных сцен;
- все возможные варианты движений и эффектов.
...
Все исходники изображений загружаются пользователем вручную в папку **sources/** в корне проекта.
...
Файлы сценариев помещаются в папку **scenarios/** в корне проекта.
...
- `movie_sizes` (list) – Размеры выходного видео в пикселях, записанные в формате [ширина, высота].
...
Визуальное и временное наполнение сцены разделено на два смысловых блока: список реплик `lines` (list), описывающих текстовые реплики в сцене, и композицию `composition` (list).
...
- `coordinates` (list) – Координаты позиции элемента относительно размеров видео в формате [x, y], где [0.0, 0.0] соответствует левому верхнему углу фонового изображения, [1.0, 1.0] – правому нижнему. Позволяет независимо позиционировать элементы при разных размерах видео. При желании возможно указание координат не в относительном формате, а в пикселях.
...
- `start` (list, optional) – Координаты начала движения. Обычно не указываются, так как по умолчанию движения начинается от координат позиционирования элемента, которые являются параметром, обязательным для указания.
